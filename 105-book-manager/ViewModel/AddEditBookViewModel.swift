...
@MainActor // live only once in one place at a time
class AddEditBookViewModel: ObservableObject { // let's the view observe this model and see changes
    ...
    init(book: PersistentBook? = nil, modelContext: ModelContext) {
        ...
        // if the book is not nil (here is a book to edit selected) we will set the properties with the values of the current selected book
        if let book {
...
// by executing outside of this... benefits
